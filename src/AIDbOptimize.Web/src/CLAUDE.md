...
- `api/*.ts`：前端契约层，隔离 `fetch` 细节；`workflow.ts` 现在同时承接发起、history、review、replay 和 review submit 的最小封装。
- `models/*.ts`：页面使用的数据模型与契约状态，避免组件直接依赖裸 JSON。
- `components/mcp/*`：MCP 管理相关的展示与操作面板。
- `components/workflow/*`：db-config workflow 的最小发起、历史、review、replay 面板。

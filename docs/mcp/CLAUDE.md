...
`docs/mcp/` 专门记录 MCP 子系统相关的长期说明文档，包括配置来源、调用链路、前端交互约定和问题排查。
...
- `postgresql-mcp-options.md`
  PostgreSQL MCP server 方案对比，解释为什么 npm 型方案更省事、原生二进制为什么更麻烦，以及本项目的推荐落地方式。
- `CLAUDE.md`
  维护本目录的边界，避免后续把运行计划、一次性排查记录和长期说明混写。

...
MCP 连接配置持久化在控制面数据库的 `McpConnections` 表中，核心字段如下：
...
PostgreSQL 默认连接已经切换到 `sgaunet/postgresql-mcp`，但不是要求机器上预装 Go 或手工下载 exe。
...
- `ServerCommand`
  `powershell`
- `ServerArgumentsJson`
  `["-NoProfile","-ExecutionPolicy","Bypass","-File","<输出目录>/postgresql-mcp-launcher.ps1"]`
- `EnvironmentJson`
  注入 `POSTGRES_URL=postgresql://...`

launcher 脚本职责：

1. 从 GitHub Releases 下载 `postgresql-mcp_0.3.0_windows_amd64.exe`
2. 缓存到 `%LOCALAPPDATA%\AIDbOptimize\tools\postgresql-mcp`
3. 使用当前环境变量中的 `POSTGRES_URL` 启动可执行文件
...
- `src/AIDbOptimize.ApiService/postgresql-mcp-launcher.ps1`
...
## 5. 默认 PostgreSQL MCP 为什么会比以前更多工具

之前默认 PostgreSQL 连接使用官方参考实现 `@modelcontextprotocol/server-postgres`，它的定位偏轻量，当前工具侧通常只有一个 `query`。

现在切到 `sgaunet/postgresql-mcp` 的目的是：

- 提供更丰富的 PostgreSQL 只读工具集合
- 更适合你当前前端的“工具列表 + 审批 + 执行器”管理模型
- 不需要你手工安装 Go 运行环境

这次切换改变的是“默认 PostgreSQL server 实现”，不是系统的调用链路。

## 6. 前端现在应该如何理解“查询工具”和“获取工具”
...
- 查询工具：读数据库
- 获取工具：实时连 MCP，再回写数据库
- 默认 PostgreSQL MCP：`sgaunet/postgresql-mcp`
- 默认 PostgreSQL 启动方式：PowerShell launcher + 自动下载缓存

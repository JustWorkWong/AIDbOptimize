...
# MySQL Diagnosis Skill

## Scope

- workflow: `db-config-optimization`
- responsibility: turn collected MySQL evidence into grounded recommendations
...
4. Output must stay consistent with the evidence reference names produced by `mysql-investigation`.

## Recommendation Rules

1. Prefer the smallest safe change that addresses the observed bottleneck or safety risk.
...
1. High confidence requires all required evidence plus direct support from at least one runtime pressure signal.
...
2. Do not restate generic MySQL tuning folklore as if it were workload-specific diagnosis.
